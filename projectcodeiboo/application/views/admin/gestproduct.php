...
<?php 
			foreach ($productos as $cos=>$valor){
				if($valor->nombre != '') //criba resultados de proveedores sin producto en tienda
					echo "
						<tr> <form method='post'/>
										<td><img src='".base_url()."pic/$valor->imagen' alt='imagen producto'/></td>
										<td>$valor->nombre</td>
										<td>$valor->n_tipos</td>
										<td>$valor->precio €</td>
										<td>$valor->caracteristicas €</td>
										<td>$valor->nom</td>
										<td><input class='des' type='submit' name='delete' value='modificar' /></td>
										<td><input class='des' type='submit' name='delete' value='Eliminar' /></td>
